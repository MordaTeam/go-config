...
func TestFallbackDecoder(t *testing.T) {
	...
	t.Run(
		"Normal_OkFirst",
		func(t *testing.T) {
			fb := config.FallbackDecoder(okDecoder(reader()), errDec)
			cfg := fbConfig{}
			err := fb.Decode(&cfg)
	...
	t.Run(
		"Normal_ErrFirst",
		func(t *testing.T) {
			fb := config.FallbackDecoder(errDec, okDecoder(reader()))
			cfg := fbConfig{}
			err := fb.Decode(&cfg)
			require.NoError(t, err)
			require.Equal(t, expOkCfg, cfg)
	...
	t.Run(
		"Fail",
		func(t *testing.T) {
			fb := config.FallbackDecoder(errDec, errDec)
			cfg := fbConfig{}
			err := fb.Decode(&cfg)
			require.Error(t, err)
			require.Empty(t, cfg)
	...
	t.Run(
		"AllNils",
		func(t *testing.T) {
			fb := config.FallbackDecoder(nil, nil)
			cfg := fbConfig{}
			err := fb.Decode(&cfg)
			require.Error(t, err)
			require.Empty(t, cfg)
